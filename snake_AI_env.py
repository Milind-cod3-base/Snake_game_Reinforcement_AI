...
class SnakeGameAI:
    ...
    def _update_ui(self):
        ...
        for pt in self.snake:  # iterating using pt below
            ...
            pygame.draw.rect(
                self.display, GREEN, pygame.Rect(
                    pt.x + 4, pt.y + 4, 12, 12))
    ...
    def _move(self, action):
        ...
        # np.array_equal(a1,a2) is true if two arrays are same shape and
        # elements
        ...
        if np.array_equal(action, [1, 0, 0]):  # we keep the current direction
            ...
            # hence we need to take next index in the clockwise direction list.
            # We dont want absolute right.

            # rotating index
            next_idx = (idx + 1) % 4  # this gives me the remainder

            # new direction
            # right turn r -> d -> l -> u (turns right, if already right, then
            # down etc.)
            new_dir = clock_wise[next_idx]

        else:  # [0,0,1]
            # if ai says to turn left, we take relative left, relative to
            # current direction
